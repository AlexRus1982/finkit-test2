let common = {
    ...
    user_delete_window: (user_id, e) => {
        ...
        if (confirm('Удалить пользователя!!! Вы уверены?')) {
            // vars
            let data = {
                user_id: user_id,
                offset: global.offset,
    ...
    user_check_fields: (event) => {
        const first_name = ge('first_name').value.trim();
        const last_name = ge('last_name').value.trim();
        
        const phone = ge('phone').value.trim().replace(/[^\d]/g, '');
        ge('phone').value = `${phone}`;

        const email = ge('email').value.trim().toLowerCase();
        ge('email').value = `${email}`;

        const save_button = ge('user_save_button');
